...
cvs.addEventListener('touchmove',e=>{e.preventDefault();
  ...
  if(fingers.size===2){ const [a,b]=[...fingers.values()]; const cur=Math.hypot(a.x-b.x,a.y-b.y); const scale=clamp(cur/startDist,0.3,3);
    ...
    const v=viewSizeWorld(); cam.x=clamp(cam.x-dx,0,CONFIG.world.w-v.w); cam.y=clamp(cam.y-dy,0,CONFIG.world.h-v.h); cur.prev={x:cur.x,y:cur.y}; }
},{passive:false});
...
addEventListener('mousemove',e=>{ if(!dragging) return; const dx=(e.clientX-lastMouse.x)/cam.z, dy=(e.clientY-lastMouse.y)/cam.z;
  const v=viewSizeWorld(); cam.x=clamp(cam.x-dx,0,CONFIG.world.w-v.w); cam.y=clamp(cam.y-dy,0,CONFIG.world.h-v.h); lastMouse={x:e.clientX,y:e.clientY};});
